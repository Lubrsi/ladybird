...
public:
    ...
    void set_frame(IsTailcall is_tailcall, ModuleInstance const& module, Vector<Value, ArgumentsStaticSize> locals, Expression const& expression, size_t arity)
    {
        ...
        // A tail call replaces the frame, so release the outgoing frame's record first;
        // otherwise a tail-call loop would grow the record region without bound.
    ...
    // Call records are per-frame block allocations in a second region; CallFrameHandle
    // saves and restores the top mark around calls.
