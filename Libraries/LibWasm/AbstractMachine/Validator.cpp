...
ErrorOr<Validator::ExpressionTypeResult, ValidationError> Validator::validate(Expression const& expression, Vector<ValueType> const& result_types, Span<CodeSection::Func const* const> callee_bodies, size_t current_function_index)
{
    ...
    expression.compiled_instructions = try_compile_instructions(expression, m_context.functions.span(), callee_bodies, current_function_index, m_context.locals.size(), m_context.imported_function_count, m_context.types.span());
    ...
    if (expression.compiled_instructions.direct && !is_constant_expression) {
        ...
        if (!has_unsupported_types && result_types.size() <= 1) {
            ...
            expression.compiled_instructions.cranelift_param_count = static_cast<u32>(m_context.functions[current_function_index].parameters().size());
