...
impl CraneliftCompiler {
    pub fn compile_to_bytes(
        ...
        // Locals are defined once the entry knows whether this is a fresh call or a tier-up
        // resume (see the tier-up split below); only declare them here.
        ...
        // The value stack lives in a fixed reservation with base/top at known Configuration
        // offsets, so pushes, pops and the size query are emitted inline instead of as helper
        // calls. The frame-entry stack usage hint check guarantees pushes cannot overflow.
        ...
        macro_rules! emit_stack_size {
            ($builder:expr) => {{
                ...
                // Values are 16 bytes; the tag stores above rely on that too.
        ...
        macro_rules! flush_vstack_to_real {
            ($builder:expr) => {{
                if max_stack_depth > 0 && sp > 0 {
                    // Batched: one top load and one top store around the value stores.
                    let cfg = $builder.use_var(config_var);
                    let top = $builder
                        .ins()
                        .load(ptr_type, MemFlags::trusted(), cfg, value_stack_top_offset);
            ...
            ($builder:expr) => {{
                if !local_vars.is_empty() {
                    ...
                    for i in 0..local_vars.len() {
                        ...
                        // An f32 var is only 4 bytes; store its boxed sign-extended form instead.
        ...
        // Initialize locals for a fresh call: parameters are loaded from the frame (the caller
        // wrote them), everything past them starts at zero. The caller does not pre-zero the
        // buffer, so the non-parameter slots must never be loaded here.
        macro_rules! init_locals_fresh {
            ($builder:expr) => {{
                if local_vars.is_empty() {
                    // Locals live in memory: store zeros over the non-parameter slots (value and tag).
        ...
        // Initialize locals for a tier-up resume: the interpreter's live locals are in the frame.
        ...
        while ip < insns.len() {
            ...
            match opc {
                ...
                | op::I64_STORE32 => {
                    let mem_idx = insn.imm3 & 0x7fff_ffff;
                    // Inline f32 stores read the F32 bank directly, skipping the i64 round-trip.
