...
ThrowCompletionOr<GC::Ptr<PropertyDescriptor>> ArgumentsObject::internal_get_own_property(PropertyKey const& property_key) const
{
...
ThrowCompletionOr<bool> ArgumentsObject::internal_define_own_property(PropertyKey const& property_key, GC::Ref<PropertyDescriptor> descriptor, GC::Ptr<PropertyDescriptor> precomputed_get_own_property)
{
    // 1. Let map be args.[[ParameterMap]].
    // 2. Let isMapped be ! HasOwnProperty(map, P).
    bool is_mapped = parameter_map_has(property_key);
