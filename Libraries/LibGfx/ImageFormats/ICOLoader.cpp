...
ErrorOr<void> ICOImageDecoderPlugin::load_ico_bitmap(ICOLoadingContext& context)
{
    ...
    if (PNGImageDecoderPlugin::sniff(context.stream)) {
        TRY(context.stream->seek(desc.offset, SeekMode::SetPosition));
        auto png_decoder = TRY(PNGImageDecoderPlugin::create(context.stream));
    ...
    TRY(context.stream->seek(desc.offset, SeekMode::SetPosition));
    auto bmp_decoder = TRY(BMPImageDecoderPlugin::create_as_included_in_ico({}, context.stream));
