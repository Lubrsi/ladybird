...
GC::Ref<PendingResponse> nonstandard_resource_loader_file_or_http_network_fetch(JS::Realm& realm, Infrastructure::FetchParams const& fetch_params, HTTP::Cookie::IncludeCredentials include_credentials, IsNewConnectionFetch is_new_connection_fetch, RefPtr<HTTP::MemoryCache> http_cache)
{
    ...
    // NOTE: The buffer is the FetchByteChannel; its watermarks are the user-agent-chosen lower and upper limits.
    //       Navigation responses start held so document loading decides when body bytes may flow at all.
    static constexpr Engine::FetchByteChannel::Watermarks network_body_watermarks { .low = 64 * KiB, .high = 256 * KiB };
    auto navigation_hold = fetch_params.has_response_body_transfer_lease()
        ? Engine::FetchBodyDeliveryGate::NavigationHold::Held
        : Engine::FetchBodyDeliveryGate::NavigationHold::None;
    auto delivery_gate = Engine::FetchBodyDeliveryGate::create(navigation_hold);
    auto wake_sink = BodyStreamWakeSink::create();
    auto channel = Engine::FetchByteChannel::create(network_body_watermarks, wake_sink, delivery_gate->create_channel_sink());
    ...
    auto on_headers_received = GC::create_function(GC::Heap::the(), [&fetch_params, pending_response, stream, request, body_pump, delivery_gate](Requests::Request* request_server_request, HTTP::HeaderList const& response_headers, Optional<u32> status_code, Optional<String> const& reason_phrase, Optional<Core::ImmutableBytes> javascript_bytecode, Optional<u64> javascript_bytecode_cache_vary_key, Requests::CameFromCache) {
        ...
        //         BodyStreamPullSource::on_wake()); record that, for consume_a_preloaded_resource().
        fetch_params.controller()->set_response_arrived();
    ...
    auto on_data_received = GC::create_function(GC::Heap::the(), [body_pump, pull_source](Requests::ResponseData data) {
        body_pump->handle_network_data(move(data));
        if (HTML::main_thread_event_loop().execution_paused())
            pull_source->deliver_while_paused();
    });
    ...
    auto on_complete = GC::create_function(GC::Heap::the(), [&realm, pending_response, body_pump, pull_source, delivery_gate, fetch_timing_info, cross_origin_isolated_capability](bool success, Requests::RequestTimingInfo const& timing_info, Optional<StringView> error_message) {
        HTML::TemporaryExecutionContext execution_context { realm, HTML::TemporaryExecutionContext::CallbacksEnabled::Yes };

        // on_teardown never fires for a request that finishes normally.
    ...
    if (network_request) {
        ...
        // The gate detaches before the cancellation reaches it.
