...
Policy Policy::parse_a_serialized_csp(Variant<ByteBuffer, String> serialized, Source source, Disposition disposition)
{
    ...
    auto tokens = MUST(serialized_string.split(';'));
    for (auto token : tokens) {
        ...
        for (auto directive_value_view : directive_value_views) {
            dbgln("got value for {}: {}", lowercase_directive_name, directive_value_view);
        ...
        auto directive = Directives::create_directive(move(lowercase_directive_name), move(directive_value));
...
Vector<Policy> Policy::parse_a_responses_content_security_policies(GC::Ref<Fetch::Infrastructure::Response const> response)
{
