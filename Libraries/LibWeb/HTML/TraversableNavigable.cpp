...
void TraversableNavigable::append_session_history_traversal_steps(GC::Ref<GC::Function<NonnullRefPtr<Core::Promise<Empty>>()>> steps)
{
...
void TraversableNavigable::append_session_history_synchronous_navigation_steps(GC::Ref<Navigable> target_navigable, GC::Ref<GC::Function<NonnullRefPtr<Core::Promise<Empty>>()>> steps)
{
...
void TraversableNavigable::apply_the_history_step(
    ...
    GC::Ref<GC::Function<void(HistoryStepResult)>> completion_steps)
{
    auto& vm = this->vm();
    // FIXME: 1. Assert: This is running within traversable's session history traversal queue.
                    ...
                    Platform::EventLoopPlugin::the().deferred_invoke(GC::create_function(vm.heap(), [&vm, populated_target_entry, potentially_target_specific_source_snapshot_params, target_snapshot_params, allow_POST, navigable, after_document_populated = GC::create_function(vm.heap(), move(after_document_populated)), user_involvement] {
                        ...
                        navigable->populate_session_history_entry_document(
                            ...
                            *potentially_target_specific_source_snapshot_params,
                            ...
                            allow_POST,
                            GC::create_function(vm.heap(), [&vm, navigable, after_document_populated, populated_target_entry]() mutable {
                                VERIFY(navigable->active_window());
    ...
    GC::Ref<GC::Function<void(CheckIfUnloadingIsCanceledResult)>> completion_steps)
{
    ...
    auto documents_to_fire_beforeunload = heap().allocate<GC::HeapVector<GC::Ref<DOM::Document>>>();
    for (auto& navigable : navigables_that_need_before_unload)
        documents_to_fire_beforeunload->elements().append(*navigable->active_document());

    // 2. Let unloadPromptShown be false.
    // 3. Let finalStatus be "continue".
    // NOTE: These will be allocated when needed.

    auto after_checking_given_traversable_continues = [this, completion_steps, documents_to_fire_beforeunload](CheckIfUnloadingCancelledContinuationState::UnloadPromptShown unload_prompt_shown) {
        ...
        for (auto document : documents_to_fire_beforeunload->elements()) {
    ...
    if (traversable) {
        ...
        if (target_entry != traversable->current_session_history_entry() && target_entry->document_state()->origin() != traversable->current_session_history_entry()->document_state()->origin()) {
            ...
            if (needs_beforeunload) {
                documents_to_fire_beforeunload->elements().remove_first_matching([&](auto& document) {
                    return document.ptr() == traversable->active_document().ptr();
                });
            }

            auto given_traversable_check_if_unloading_cancelled_continuation_state = heap().allocate<CheckIfUnloadingCancelledContinuationState>(/* remaining= */ 1, GC::create_function(heap(), [completion_steps, after_checking_given_traversable_continues = GC::create_function(heap(), move(after_checking_given_traversable_continues))](CheckIfUnloadingIsCanceledResult final_status, CheckIfUnloadingCancelledContinuationState::UnloadPromptShown unload_prompt_shown) {
                ...
                after_checking_given_traversable_continues->function()(unload_prompt_shown);
            }));
    ...
    after_checking_given_traversable_continues(CheckIfUnloadingCancelledContinuationState::UnloadPromptShown::No);
}
...
{
    check_if_unloading_is_canceled(move(navigables_that_need_before_unload), {}, {}, {}, completion_steps);
}
...
void TraversableNavigable::traverse_the_history_by_delta(int delta, GC::Ptr<DOM::Document> source_document)
{
    ...
    append_session_history_traversal_steps(GC::create_function(heap(), [this, delta, source_snapshot_params, initiator_to_check, user_involvement] {
        ...
        apply_the_traverse_history_step(all_steps[target_step_index], source_snapshot_params, initiator_to_check, user_involvement, GC::create_function(heap(), [signal_to_continue_session_history_processing](HistoryStepResult) {
            signal_to_continue_session_history_processing->resolve({});
        }));
...
void TraversableNavigable::update_for_navigable_creation_or_destruction(GC::Ref<GC::Function<void(HistoryStepResult)>> completion_steps)
{
...
void TraversableNavigable::apply_the_reload_history_step(UserNavigationInvolvement user_involvement, GC::Ref<GC::Function<void(HistoryStepResult)>> completion_steps)
{
    // 1. Let step be traversable's current session history step.
    auto step = current_session_history_step();
...
void TraversableNavigable::apply_the_push_or_replace_history_step(int step, HistoryHandlingBehavior history_handling, UserNavigationInvolvement user_involvement, GC::Ref<GC::Function<void(HistoryStepResult)>> completion_steps)
{
...
void TraversableNavigable::apply_the_traverse_history_step(int step, GC::Ptr<SourceSnapshotParams> source_snapshot_params, GC::Ptr<Navigable> initiator_to_check, UserNavigationInvolvement user_involvement, GC::Ref<GC::Function<void(HistoryStepResult)>> completion_steps)
{
...
void finalize_a_same_document_navigation(GC::Ref<TraversableNavigable> traversable, GC::Ref<Navigable> target_navigable, GC::Ref<SessionHistoryEntry> target_entry, GC::Ptr<SessionHistoryEntry> entry_to_replace, HistoryHandlingBehavior history_handling, UserNavigationInvolvement user_involvement, GC::Ref<GC::Function<void()>> on_complete)
{
    ...
    traversable->apply_the_push_or_replace_history_step(*target_step, history_handling, user_involvement, GC::create_function(traversable->heap(), [on_complete](TraversableNavigable::HistoryStepResult) {
