...
// Recurses through wrapper templates and record fields so nested cases like
// Optional<Vector<GC::Ptr<T>>> and named wrapper structs are caught.
static bool type_has_unrooted_gc_container(clang::QualType type, std::set<clang::CXXRecordDecl const*>& visited)
{
    ...
    if (auto const* specialization = type->getAs<clang::TemplateSpecializationType>()) {
        ...
        // RootedExecutionContext registers its own edges with the VM.
    ...
    // Recurse into struct/class fields. Cell subclasses and GC:: types handle their own visiting.
